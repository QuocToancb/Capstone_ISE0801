...
//
//                String url = "http://org2.s1.mp3.zdn.vn/c4d8fe9b5cdfb581ecce/8552917280299845060?key=X1ePdJDvOi5XQnekzWo62w&expires=1478914575&filename=Nguoi%20Va%20Ta%20-%20Rhymastic%20Thanh%20Huyen.mp3"; // your URL here
//
...
//                                fillBuffer(videoQuadTextureCoordsTransformedChips));
    ...
    private final Handler handlerToast = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.arg1 == 1) {
                Toast.makeText(mActivity.getApplicationContext(), "Audio", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(mActivity.getApplicationContext(), "Video", Toast.LENGTH_SHORT).show();
            }

        }
    };
    private final Handler handlerButton = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.arg1 == 1) {
                mActivity.targetFound();
            } else {
                mActivity.targetLost();
            }
