...
Class Model extends CI_Model
{
	...
	function checkLogin($user, $pass)
	{
		...
		if ($query->num_rows() > 0 ) //Nếu trả về dữ liệu (trong db có bản ghi của tk và mk đó)
		{
			...
			//$temp = $this->selectAll('taikhoan_nhomnguoidung', array('NHANVIEN_id'=>$detail['NHANVIEN_id']));
			$this->session->set_userdata('nhomnguoidungss', $detail);	
	...
	function checkRole($NHOMNGUOIDUNG_id)
	{
		...
		{
				if ( is_array($NHOMNGUOIDUNG_id))//Nếu người đăng nhập có nhiều quyền thì: ghép chuỗi rồi đưa vào câu truy vấn
				{
					$nhomnguoidung = '';
					foreach ($NHOMNGUOIDUNG_id as $val)
						$nhomnguoidung .= ','.$val['NHOMNGUOIDUNG_id'];
					$nhomnguoidung = trim($nhomnguoidung, ','); //cắt bỏ dấu phẩy ở hai đầu
				} else $nhomnguoidung = $NHOMNGUOIDUNG_id;
				$query=$this->db->query("SELECT * FROM nhomnguoidung_quyentruycap_thaotac WHERE NHOMNGUOIDUNG_id IN ($nhomnguoidung) AND QUYENTRUYCAP_id IN (SELECT QUYENTRUYCAP_id FROM quyentruycap WHERE QUYENTRUYCAP_function='$func')");
		...
		else
		{
			if ( is_array($NHOMNGUOIDUNG_id))//Nếu người đăng nhập có nhiều quyền thì: ghép chuỗi rồi đưa vào câu truy vấn
			{
				...
				foreach ($NHOMNGUOIDUNG_id as $val)
					$nhomnguoidung .= ','.$val['NHOMNGUOIDUNG_id'];
				$nhomnguoidung = trim($nhomnguoidung, ','); //cắt bỏ dấu phẩy ở hai đầu
				$query=$this->db->query("SELECT * FROM nhomnguoidung_quyentruycap_thaotac WHERE NHOMNGUOIDUNG_id  IN ($nhomnguoidung) AND THAOTAC_id IN (SELECT THAOTAC_id FROM thaotac WHERE THAOTAC_title='$act') AND QUYENTRUYCAP_id IN (SELECT QUYENTRUYCAP_id FROM quyentruycap WHERE QUYENTRUYCAP_function='$func')");
	...
	function loadMenu($url, $NHOMNGUOIDUNG_id, $parent=0)
	{
		...
		$query = $this->db->query("SELECT QUYENTRUYCAP_title, QUYENTRUYCAP_function, QUYENTRUYCAP_parent, QUYENTRUYCAP_id, span_class
						  FROM quyentruycap WHERE QUYENTRUYCAP_id IN
						  (SELECT QUYENTRUYCAP_id FROM nhomnguoidung_quyentruycap_thaotac 
						      WHERE NHOMNGUOIDUNG_id IN ($nhomnguoidung) ) 
		...
		if (is_array($result) && count($result)>0)
		{
				...
				{
					if ($item['QUYENTRUYCAP_function'] == $active) echo '<li class="active">'; else echo "<li>";		
					echo '<a href="'.$url.'index.php/main/'.$item['QUYENTRUYCAP_function'].'"><span class="'.$item['span_class'].'"></span>'.$item['QUYENTRUYCAP_title'].'</a>';
					echo '</li>';
				}
			if ( is_array($NHOMNGUOIDUNG_id))//Nếu người đăng nhập có nhiều quyền thì: ghép chuỗi rồi đưa vào câu truy vấn
			{
			...
			$query2 = $this->db->query("SELECT QUYENTRUYCAP_title, QUYENTRUYCAP_function, QUYENTRUYCAP_parent, QUYENTRUYCAP_id, span_class
						  FROM quyentruycap WHERE QUYENTRUYCAP_id IN
						  (SELECT QUYENTRUYCAP_id FROM nhomnguoidung_quyentruycap_thaotac 
						      WHERE NHOMNGUOIDUNG_id IN ($nhomnguoidung)) 
						  AND QUYENTRUYCAP_parent=$parent AND QUYENTRUYCAP_function=''");
			...
			if (is_array($result2) && count($result2)>0)
			{
				foreach ($result2 as $stt => $item)
				{
					...
					$this->loadMenu($url, $NHOMNGUOIDUNG_id, $item['QUYENTRUYCAP_id']);
					echo "</ul></div><div></li>";
				}
			}
		}
